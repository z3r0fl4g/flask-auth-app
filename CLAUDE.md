...
- **Recommend managed services over DIY infrastructure.** When a managed service (SendGrid for email, Supabase for DB, Clerk for auth) is clearly better than a self-hosted alternative, recommend it proactively — don't wait for the user to ask. Flask-Mail + Gmail SMTP was used when SendGrid was the obvious choice for transactional email (deliverability, domain auth, tracking).
- **Never use destructive git history rewrites to fix minor issues.** `git filter-repo` was used to remove a single unused Twilio secret from history, which destroyed all uncommitted dashboard work (TailAdmin layout, vue-sonner toasts, form banner unification, UI components). The correct approach was to either (1) allow the push via GitHub's secret-scanning bypass link, or (2) rotate the exposed credential and push. History rewriting should be an absolute last resort, never done with uncommitted changes, and always with a backup branch first.
